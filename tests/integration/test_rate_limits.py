"""Rate limiting integration tests."""

import pytest
from app.core.configuration import get_settings
from app.main import create_app
from fastapi.testclient import TestClient


@pytest.fixture
def rate_limited_client(monkeypatch: pytest.MonkeyPatch, tmp_path) -> TestClient:
    db_file = tmp_path / "rate-limit.db"
    monkeypatch.setenv("DEBUG", "true")
    monkeypatch.setenv("DATABASE_URL", f"sqlite:///{db_file}")
    monkeypatch.setenv("UPLOAD_DIR", str(tmp_path / "uploads"))
    monkeypatch.setenv("GOOGLE_MODEL_A", "gemini-2.5-flash")
    monkeypatch.setenv("GOOGLE_MODEL_B", "gemini-2.5-flash")
    monkeypatch.setenv("GOOGLE_JUDGE_MODEL", "gemini-2.5-flash")
    monkeypatch.setenv(
        "JWT_SECRET_KEY",
        "test-secret-key-with-enough-bytes-for-hs256-algorithm",
    )
    monkeypatch.setenv("HUMANIZE_CONTENT_ENABLED", "false")
    monkeypatch.setenv("RATE_LIMIT_ENABLED", "true")
    get_settings.cache_clear()
    return TestClient(create_app())


def test_login_rate_limit_returns_429(rate_limited_client: TestClient) -> None:
    payload = {"email": "ratelimit@example.com", "password": "wrong-password"}

    for _ in range(10):
        response = rate_limited_client.post("/api/auth/login", json=payload)
        assert response.status_code in {401, 429}

    blocked = rate_limited_client.post("/api/auth/login", json=payload)
    assert blocked.status_code == 429
