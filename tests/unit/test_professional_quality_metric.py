...
async def test_professional_quality_hybrid_score() -> None:
    ...
    assert "Subject line detected." in result.details


@pytest.mark.asyncio
async def test_professional_quality_subject_missing_in_body_only_input() -> None:
    mock_client = MagicMock()
    mock_client.generate_content = AsyncMock(
        return_value="GRAMMAR: 5\nCLARITY: 4\nOPENING: 4"
    )
    ...
    result = await metric.score(
        MetricInput(
            generated_email="Dear Ms. Lee,\n\nThe project remains on track."
        )
    )

    assert "Subject line missing." in result.details
